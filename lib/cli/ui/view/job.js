...
var jobContent = function jobContent(status) {
  return ' ' + status.name + ': run ' + status.id + ': ' + (_lodash2.default.isUndefined(status.endTime) ? 'pending' : _lodash2.default.isUndefined(status.error) ? 'ok' : status.error);
};
// eslint-disable-next-line max-len
var jobBg = function jobBg(status) {
  return _lodash2.default.isUndefined(status.endTime) ? 'yellow' : _lodash2.default.isUndefined(status.error) ? 'green' : 'red';
};
