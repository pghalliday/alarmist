...
var _asciichart = _interopRequireDefault(require("asciichart"));
...
const STATUS_COLORS = ['green', 'yellow', 'red']; // check if there is a non zero range of values
...
function jobContent(status, entry) {
  ...
  if (_lodash.default.isUndefined(status.endTime)) {
    if (entry) {
      if (entry.message) {
    ...
    message = _lodash.default.isUndefined(status.error) ? 'ended' : status.error;
  }
...
class Metric extends _entry.default {
  constructor() {
    super();
    this.expanded = false;
    this.chart = _blessed.default.box(_lodash.default.cloneDeep(_constants.CHART_PROPERTIES));
    this.chart.hide();
    this.clear();
  }
  ...
  _update(status) {
    const subsetLength = this.chart.width - _constants.CHART_PADDING.length - 2; // istanbul ignore else

    if (subsetLength > 1) {
      // the last line should always be incomplete so
      // take the last subsetLength complete lines and parse the values
      const subset = status.lines.slice(-subsetLength - 1, -1).map(parseValue); // asciichart will fall over if the range of values has zero size

      if (rangeIsGood(subset)) {
        this.chart.setContent(_asciichart.default.plot(subset, {
